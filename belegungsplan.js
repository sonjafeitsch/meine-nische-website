...
fetch('belegung.json').then(response => {
  ...
  document.getElementById('previous-week-button').onclick = () => {
    ...
    if(dayjs().startOf('isoWeek').unix() >= selectedDate.startOf('isoWeek').unix()) {
...
function fillSchedule(selectedTime, schedule) {
  ...
  document.getElementById('schedule-start').innerHTML = start.format('DD.MM.YYYY')
  ...
  schedule
    ...
    .forEach(course => fillCell(course, dayjs(course.day).format('dddd').toLowerCase()))
}
